...
class Assembly:
    ...
    def equivalent_permeability(self, faces_local_ids, coarse_volume):
        """
    ...
    def assembly_local_problem(self, coarse_volume):
        """
        ...
        adjacent_volumes = self.coarse.elements[coarse_volume].faces.bridge_adjacencies(faces_local_ids, 2, 3) # Local IDs from both the neighbors from each of the internal faces
        adjacent_volumes_flatten = adjacent_volumes.flatten()
        neighbors_centers = np.reshape(self.coarse.elements[coarse_volume].volumes.center[adjacent_volumes_flatten], newshape = (len(faces_local_ids), 6))
